...
{'num_steps': 233, 'data_to_plot': None, 'label': ''},
...
for time_step in range(1, nsteps + 1):
    ...
    #__________onda sinusoidal__________
    #Put a sinusoidal soft source at the low end
    #pulse = np.sin(2 *np.pi * freq_in * dt * time_step)
    #ex[5] = pulse + ex[5]

    #__________wave packet__________
    pulse = exp(-0.5 * ((t0 - time_step) / spread) ** 2)*np.sin((time_step - t0))
    ex[5] = pulse + ex[5]
